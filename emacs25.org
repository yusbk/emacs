...
#+BEGIN_SRC emacs-lisp
  (use-package polymode
    ...
    (key-chord-define-global "zm" #'hydra-ybk-rmd/body)
...
* HTML and Webpage
** Browser
Buka link di browser guna =C-x w= kekunci yang asalnya berfungsi ~other-window~. Untuk
...
#+BEGIN_SRC emacs-lisp
  ...
  (key-chord-define-global "zb" #'ybk-map-move/body)
#+END_SRC
** Org keymap
Hydra keymap untuk =org=
#+BEGIN_SRC emacs-lisp :tangle yes
  (defhydra ybk-keymap-org (:hint nil)
    "
    ^Org-keymap [F6] or zd^   [tekan _q_ utk KELUAR]
    ^-------------------------------------------------
    _y_: my agenda      _f_: kamus             _i_: insert cite           _x_: export pdf    _<down>_: next heading
    _r_: refile agenda  _k_: tukar bahasa      _l_: insert internal link  _z_: save as pdf   _<up>_  : previous heading
    _w_: today          _t_: google-translate  _v_: org option (sparse)   _n_: meeting note
    _d_: TODO           _p_: gambar/pic        _c_: bibtex                _m_: meeting done       _s_: insert src-block
...
#+END_SRC
** Prog keymap
Hydra keymap untuk program seperti penggunaan =[F12]= cuma lebih ringkas
#+BEGIN_SRC emacs-lisp
  (defhydra hydra-ybk-prog (:hint nil)
    "
    ^Programming keymap: [F12] or zp^
    ^----------------------------------
    _p_: Projectile      _a_: start exp    _g_: Magit
    _f_: Find project    _e_: end exp      _w_: Send2browser
    _y_: Yasnippet       _u_: unwrap exp   _o_: Origami
    _-_: Del unuse yas.  _x_: kill exp     _c_: Cleanup/tabbify
  ...
  (key-chord-define-global "zp" #'hydra-ybk-prog/body)
#+END_SRC
