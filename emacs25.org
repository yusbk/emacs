...
#+END_SRC
** R keymap [F9]
Keymap untuk penggunaan berkaitan R
#+BEGIN_SRC emacs-lisp
  (bind-keys :prefix "<f9>"
...
#+END_SRC
* Go to settings file
Fungsi untuk menyenangkan membuka fail-fail yang selalu digunakan atau
dirujuk. Contohnya fail aturcara Emacs =emacs25.org= dan shortcuts yang selalu dipakai
=shortcuts=.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun ybk-shortcut ()
    "Buka fail shortcuts"
    (interactive)
    (find-file (concat directory-sendiri "shortcuts.org")))

  (bind-key "a" #'ybk-shortcut 'ybk/map)
#+END_SRC
* Info+
...
#+BEGIN_SRC emacs-lisp
  (use-package winum
    ...
    :defer 1
    :bind (:map ybk/move-map
                ("1" . winum-select-window-by-number))
    :init
    (setq winum-keymap
...
Program =org-mode= yang dipakai disini diklon dari ~GitHub ~ melalui [[git clone
git://orgmode.org/org-mode.git]]. =Fail-fail yang ditetapkan di folder =org-agenda-fail=
perlu dibuat sendiri terlebih dahulu sebelum memakai fungsi =C-c c=.
...
#+BEGIN_SRC emacs-lisp
  (use-package org
    :load-path "~/Dropbox/lisp/org-mode/"
    :mode (("\\.txt$" . org-mode)
           ("\\.org$" . org-mode))
    :config
...
#+END_SRC
** Slides Presentation
Untuk buat slides pembentangan menggunakan =Reveal.js=. Untuk memakai Reveal
menggunakan org-mode, harus memuatkan ~org-raveal~ dari [[https://github.com/yjwen/org-reveal][GitHub]] atau Elpa. Selain itu
perlu dapatkan =Reveal.js= dari [[http://hakim.se/][Hakim El Hattab]] di [[https://github.com/hakimel/reveal.js][GitHub]].

=Reveal.js= boleh diletakkan di folder yang sama setiap kali menyediakan
slides. Sekiranya =Reveal.js= disimpan di folder yang sama, tetapkan =org-reveal-root=
dengan hanya ="."= dan sekiranya difolder yang lain, tetapkan dengan
=file:///tempat/folder/reveal.js=. Sekiranya tidak menbuat salinan =Reveal.js= maka boleh
meletakkan =#+REVEAL_ROOT:= di org-file slide yang sedang dibuat.
...
#+BEGIN_SRC emacs-lisp
  (use-package ox-reveal
    :ensure org
    :load-path "~/Dropbox/lisp/org-reveal/"
    :init
    ;; tetapkan lokasi
...
#+BEGIN_SRC emacs-lisp
  (use-package google-traslate-default-ui
    :ensure google-translate
