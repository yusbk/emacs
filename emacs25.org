...
app-folder. Untuk larikan fungsi ini menggunakan =C-c s= mesti berada di app-folder.
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; (global-set-key (kbd "C-c s") 'run-shiny-app)
  (global-set-key (kbd "<f9>") 'run-shiny-app)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;== key-binding
  (key-seq-define-global "jk" 'hydra-ybk-r-debug/body)
