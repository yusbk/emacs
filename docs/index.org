...
** Movement
Hanya guna =C-n= untuk aktifkan kekuci. =Dimatikan=
...
#+BEGIN_SRC emacs-lisp
  (defun ybk/emacs-file ()
    ...
    (interactive)
    (find-file (concat user-emacs-directory "emacs26.org")))
...
#+END_SRC
** Exit
Save buffer and exit. =Dimatikan=
#+BEGIN_SRC emacs-lisp :tangle no
  (bind-key "9" #'save-buffers-kill-emacs 'ybk/map )
#+END_SRC
** Scratch buffer
Untuk buka ~scratch~ boleh dicapai melalui [[*General keymap %5Bjm%5D][General keymap]] atau [[*My personal hydra keymaps][personal hydra keymaps]].
#+BEGIN_SRC emacs-lisp
...
chunck cursor akan terus keluar chunck kalau tidak di 'save'. =Dimatikan=
...
** Line number
Tunjukkan line number. =Dimatikan=
...
Diambil di [[https://github.com/sandhu/emacs.d/blob/master/config/008-ui-config.el][sini]]. =Dimatikan=
...
*** Moe-theme
Untuk aturcara boleh baca di [[https://github.com/kuanyui/moe-theme.el][GitHub]]. =Dimatikan=
...
Untuk cari atau cari dan tukar teks di direktori. Diambil dari [[https://github.com/xahlee/xah-find][GitHub]]. Antara fungsi
yang boleh dipakai:
...
** Tags
Utk buat project etags atau gtags. Pertama pasangkan =ctags=.

#+BEGIN_SRC sh
  sudo apt install ctags
#+END_SRC

Kemudian pasang =gtags= dari [[https://github.com/leoliu/ggtags][Melpa]].
...
#+BEGIN_SRC emacs-lisp
  (use-package ggtags
    ...
    (add-hook 'c-mode-common-hook
              (lambda ()
                (when (derived-mode-p 'c-mode 'c++-mode 'java-mode 'ess-r-mode)
                  (ggtags-mode 1))))
    )
#+END_SRC
...
- Pasang pakej-pakej lain melalui terminal atau larikan di sini

#+BEGIN_SRC sh
  sudo apt-get install autoconf automake g++ gcc libpng-dev libpoppler-dev libpoppler-glib-dev libpoppler-private-dev zlib1g-dev make pkg-config
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package org-ref
    ...
    :bind*
    (("<f5> i" . org-ref-helm-insert-cite-link)
     :map bibtex-mode-map
    ...
    ;;pakai latexmk utk prosess - sama seperti fungsi di atas tapi tidak berhasil proses reference list
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package flyspell
    ...
    (key-seq-define-global "jk" #'hydra-ybk-kamus/body)
    ...
    (add-hook 'text-mode-hook 'flyspell-mode) ;nok til å aktivere org og latex
    ...
    ;; Deaktivere i change-log-mode og log-edit-mode
    ...
    ;; Periksa nahu (grammar) selepas menyimpan (save) tekst - di markdown dan org
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package auto-lang
    ...
    (add-hook 'polymode-minor-mode-hook 'ybk/auto-lang)
    (add-hook 'markdown-mode-hook 'ybk/auto-lang)
    )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package mu4e
    ...
    :bind (("C-c e " . ybk-mu4e-opt/body)
           ;; ("C-c e" . mu4e)
           ("<f3>" . compose-attach-marked-files)
           )
    ...
    (defhydra ybk-mu4e-opt (:color blue)
      ...
      ("c" compose-attach-marked-files "compose w/attach")
    ...
    ;; guna browser bila ada table. mu4e-view-action tekan 'a' = action
...
** Suspend-frame
Emacs freez bila tekan =C-x C-z= atau =C-z=. Kekunci ini boleh dimatikan dengan
meletakkan di aturcara emacs:
#+BEGIN_EXAMPLE
(global-unset-key (kbd "C-x C-z"))
#+END_EXAMPLE

Sekiranya kekunci ini tidak dimatikan, bila frame freeze boleh gunakan komando ini di
terminal:

#+BEGIN_EXAMPLE
xkill -frame $emacs_pid
xkill -frame $emacs
#+END_EXAMPLE

Kemudian mouse-click Emacs frame.
...
#+BEGIN_SRC emacs-lisp
  (defhydra hydra-ybk-general (:hint nil)
    "
     ...
     ^-------------------------------
     _y_: ybk-init  _g_: highlight        _s_: stackOverflow  _b_: buffer baru        _e_: musics (emms)   _x_: save buffers & exit
     _h_: heading   _l_: warna highlight  _r_: rss            _n_: file explore [F4]  _u_: weather (metno) _c_: key-chord-mode
     _t_: theme     _m_: menu bar         _o_: scratch        _k_: delete file/buffer _U_: weather update
...
#+BEGIN_SRC emacs-lisp
  (defhydra ybk-map-move (:hint nil)
    ...
    _w_: transpose window  _o_: imenu        _t_: bookmark-goto   _j_: reg jump      _n_: mark next   _A_: mark show all
    _f_: trans. frame C-t  _g_: goto         _J_: bookmark jump   _l_: bookmark list _p_: mark prev   _x_: mark remove all
    _b_: move buffer       _s_: search
    "
    ("+" resize-window)
...
#+BEGIN_SRC emacs-lisp
  (defhydra hydra-ybk-prog (:hint nil)
    ...
    _p_: find project  _e_: end exp      _y_: Yasnippet                   _d_: Ediff            _R_: debug R (jx)
...
** Sweave keymap [js]
Hydra untuk polymode guna masa knitr.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defhydra hydra-ybk-poly (:hint nil)
    "
       ^Chunk^                        ^Evaluate^          ^Knit^
       ^----------------------------------------------------------------------------------
       _i_: insert      _f_: Fold      _r_: chunk/region   _E_: set exporter   _q_: KELUAR
       _c_: insert arg  _a_: Fold all  _s_: to point       _e_: export
       _k_: kill        _u_: Unfold    _z_: from point     _w_: weave
       _n_: next                     _b_: buffer         _d_: Create html
       _p_: previous
    ...
    ("s" polymode-eval-buffer-from-beg-to-point)
    ("z" polymode-eval-buffer-from-point-to-end)
    ...
    ("r" polymode-eval-region-or-chunk)
  ...
  ;;==key ikut urutan
  (key-seq-define-global "js" 'hydra-ybk-poly/body)
  ;; (key-chord-define-global "js" #'hydra-ybk-poly/body)
#+END_SRC
